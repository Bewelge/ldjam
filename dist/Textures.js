...
import { ns, PI05, rndFloat, rndInt, Vec2 } from "./util.js";
    ...
    getTexture(key, p) {
        ...
        if (imgs) {
            let ind = p
                ...
                : rndInt(0, imgs === null || imgs === void 0 ? void 0 : imgs.length);
...
let theTexture;
export const initTextures = () => __awaiter(void 0, void 0, void 0, function* () {
    theTexture = new Textures();
    yield theTexture.generateTextures();
    yield theTexture.loadTextures();
});
export function getTexture(key, p) {
...
export function generateBrokenTexture(amount) {
    ...
    for (let i = 0; i < amount; i++) {
        let wh = rndInt();
        let rndP = new Vec2(rndInt(0, 1 + TILE_SIZE / 16) * 16, rndInt(0, 1 + TILE_SIZE / 16) * 16);
        Vec2.random(-TILE_SIZE, TILE_SIZE * 2, TILE_SIZE * 2);
        ct.fillRect(rndP.x, rndP.y, 1 * rndInt(1, 3) + wh * rndInt(15, 40), 8 * rndInt(1, 3) + (1 - wh) * rndInt(15, 40));
    }
    // document.body.appendChild(cnv)
    cnv.id = "typeKey";
    return cnv;
}
export function generateTexture(typeKey) {
    let cnv = document.createElement("canvas");
    cnv.width = TILE_SIZE * 2;
    cnv.height = TILE_SIZE * 2;
    let ct = cnv.getContext("2d");
    ...
    if (TileTypes[typeKey].type == "ore") {
        ...
        for (let i = 1; i < TILE_SIZE * 2 - 1; i += step) {
            for (let j = 1; j < TILE_SIZE * 2 - 1; j += step) {
                ...
                let xRat = Ease.easeInOutQuad(1 - Math.abs(i - TILE_SIZE) / (TILE_SIZE - 2));
                let yRat = Ease.easeInOutQuad(1 - Math.abs(j - TILE_SIZE) / (TILE_SIZE - 2));
                if (Math.abs(nois) < 0.5 * xRat * yRat) {
                    for (let k = 0; k < 1; k++) {
                        ...
                        ct.fillRect(p.x, p.y - offset - bitSize / 4 - bitSize / 4, rndW, rndH);
                    }
                }
            }
        }
