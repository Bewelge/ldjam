...
import { inventory, noiseOffset, player, PLAYER_SIZE, TILE_SIZE, } from "./main.js";
import { addOreGain } from "./OreGainSys.js";
import { getBrokenTexture0, getBrokenTexture1, getTexture } from "./Textures.js";
import { ns, rndFloat, transformWorldToRenderP, Vec2 } from "./util.js";
...
export const TileTypes = {
    ...
    MITHRIL: {
        ...
        test: (p, rangeRat) => {
            let ns0 = (ns(p, noiseOffset.copy().multiply(1255.23), 1.15) + 1) / 2;
            let ns1 = (ns(p, noiseOffset.copy().multiply(1.623), 0.09) + 1) / 2;
            return (ns0 < 0.15 * (0.2 + 0.8 * Ease.easeOutQuad(1 - (rangeRat - 0.5) / 0.5)));
        },
    ...
    GOLD: {
        ...
        test: (p, rangeRat) => {
            ...
            return (ns0 <
                0.007 +
                    Math.max(0.05, 0.013 * Math.log(Math.max(1, p.y * 0.01)) * Math.pow(rangeRat, 2)));
        },
    ...
    SAPPHIRE: {
        ...
        resource: "SAPPHIRE",
        baseValue: 250,
        resourceAmount: () => 1 + Math.floor(2 * Math.pow(rndFloat(), 3)),
        isBreakable: () => true,
        test: (p) => {
...
export class Tile {
    ...
    getTexture(type, p) {
        if (!this.texture) {
            this.texture = getTexture(type, p);
        }
        return this.texture;
    }
...
export class Grid {
    ...
    gridToPixel(gridX, gridY) {
        return transformWorldToRenderP(new Vec2(gridX * TILE_SIZE, gridY * TILE_SIZE));
        // {
    ...
    render(ctx, playerP) {
        ...
        for (let y = Math.max(0, playerCoordY - 20); y < Math.min(this.height, playerCoordY + 20); y++) {
            for (let x = Math.max(0, playerCoordX - 20); x < Math.min(this.width, playerCoordX + 20); x++) {
                const pixel = this.gridToPixel(x, y);
                this.tiles[y][x].render(ctx, pixel.x - playerP.x, pixel.y - playerP.y, TILE_SIZE);
            }
        }
    }
            ...
            xLoop: for (let x = 0; x < this.width; x++) {
                for (let { key: oreKey, value: oreType } of Ores) {
                    ...
                    if (range) {
                        ...
                        if (min < y && max > y) {
                            if (test) {
                                if (test(new Vec2(x, y), (y - range.min) / (range.max - range.min))) {
                                    this.setTile(x, y, oreKey);
                                    continue xLoop;
                                }
                            }
                            else {
                                let ironHtMod = (1 -
                                    Ease.easeInQuad(Math.abs(((y - min) / (max - min) - 0.5) / 0.5))) *
                                    0.25;
                                let nsIron = (ns(new Vec2(x + min * max, y), noiseOffset, 1) + 1) / 2;
                                if (nsIron < ironHtMod) {
                                    this.setTile(x, y, oreKey);
                                    continue xLoop;
                                }
