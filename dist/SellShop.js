...
export class SellShop extends AbstractShop {
    ...
    sellResource(inventory, type, amount) {
    ...
    sellAll() {
        ...
        Object.keys(TileTypes).forEach(type => {
            totalValue += this.sellAllOfType(type);
        });
        return totalValue;
    }
    ...
    updateContent() {
        ...
        let totalValue = 0;
        Object.keys(TileTypes).forEach(type => {
            const { color, name, baseValue = 0 } = TileTypes[type];
            const amount = inventory.getResourceAmount(type);
            ...
            if (amount > 0) {
                ...
                const resourceItem = this.createElement("div", isSelected ? "resource-item resource-item-selected" : "resource-item");
                ...
                const amountElement = this.createElement("div", "resource-amount", `Amount: ${amount}`);
                infoDiv.appendChild(nameElement);
                infoDiv.appendChild(amountElement);
                // Value info
                const valueDiv = this.createElement("div", "resource-value-container");
                const valueElement = this.createElement("div", "resource-value", `Value: ${baseValue}`);
                const totalElement = this.createElement("div", "resource-total", `Total: ${total}`);
        ...
        if (this.selectedResource !== null) {
            ...
            if (amount > 0) {
                const sellSelectedButton = this.createElement("button", "sell-button sell-selected", `Sell ${amount} Selected`);
        ...
        if (totalValue > 0) {
            const sellAllButton = this.createElement("button", "sell-button sell-all", `Sell All (${totalValue})`);
        ...
        // Empty state message
        if (totalValue === 0) {
            const emptyMessage = this.createElement("div", "empty-message", "Your inventory is empty. Go mining to find resources!");
