import { CANVAS_HEIGHT, CANVAS_WIDTH, PLAYER_SIZE, } from "./main.js";
...
export class Player {
    ...
    update() {
        ...
        else if ((this.keysDown["s"] || this.keysDown["ArrowDown"]) &&
            this.onGround &&
            !this.isJumping) {
            isMoving = true;
            this.motion._y = Math.min(Math.max(playerSpeed, this.maxDownSpeed), this.motion._y + playerSpeed);
    ...
    getTexture() {
        if (!this.texture) {
            ...
            ct.drawImage(getTexture("PLAYER_01"), 0, 0, cnvS, cnvS);
        }
        return this.texture;
    }
    getDrillTexture() {
        if (!this.drillTexture) {
            ...
            cnv.height = cnvS;
            let ct = cnv.getContext("2d");
            ct.drawImage(getTexture("DRILL"), 0, 0, cnvS, cnvS);
        }
    ...
    getDrillTexture2() {
        if (!this.drillTexture2) {
            ...
            cnv.height = cnvS;
            let ct = cnv.getContext("2d");
            ct.drawImage(getTexture("DRILL"), cnvS, 0, -cnvS, cnvS);
        }
    ...
    render(ctx) {
        ...
        getTexture(("PLAYER_0" + ((Math.floor(this.walkTicker * 0.5) % 4) + 1)));
        ...
        let headT = getTexture("PLAYER_HEAD");
        let torsoT = getTexture("PLAYER_TORSO");
        let armT = getTexture("PLAYER_ARM");
        let legT = getTexture("PLAYER_LEG");
        ...
        let dirRat = Math.min(1, Math.max(0, Math.abs(this.direction - PI05) / PI05));
        ...
        if (text) {
            ...
            ctx.drawImage(text, (this.drillTicker % 5) * 0 - PLAYER_SIZE / 6, 
            // PLAYER_SIZE / 4 +
            // dirRat * PLAYER_SIZE * 0.2 +
            (-PLAYER_SIZE * 0.75) / 2, PLAYER_SIZE * 0.75, PLAYER_SIZE * 0.75);
        }
