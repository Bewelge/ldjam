...
import { initSounds, playSound } from "./SoundManager.js";
...
import { PI, PI05, rndInt, seedRndGen, transformWorldToRenderP, Vec2, } from "./util.js";
let tileSheet = {
    tile_width: 256,
    tile_height: 256,
    tiles: Array(16)
...
(function (GameState) {
    ...
    GameState[GameState["START_GAME"] = 1] = "START_GAME";
    GameState[GameState["START_DAY"] = 2] = "START_DAY";
...
export let upgradeManager = new UpgradeManager();
export let inventory = new Inventory(15);
export let particles = [];
export let upgradeShop = new UpgradeShop();
export let sellShop = new SellShop();
...
class Game {
        ...
        switch (this.state) {
            ...
            case GameState.RUNNING:
                ...
                this.tilesToKill.forEach(coord => {
                    ...
                    oldTile === null || oldTile === void 0 ? void 0 : oldTile.triggerDeath(coord);
    ...
    render() {
        ...
        if (player.position.distanceTo(shopPos) < 1000) {
            ...
            this.ctx.drawImage(getTexture("SHOP"), shopP.x - player.position.x - shopSize / 2, shopP.y - player.position.y - shopSize + 12, shopSize, shopSize);
        }
    ...
    handleCollisions() {
        ...
        const tileDown = this.grid.getTile(playerX, Math.floor((playerP.y + PLAYER_SIZE / 2) / TILE_SIZE + 0.1));
        if ((tileDown === null || tileDown === void 0 ? void 0 : tileDown.type) == "EMPTY") {
        ...
        // 	"red",
        // )
        const playerNextX = Math.floor((playerP.x + (PLAYER_SIZE / 2) * Math.sign(motion.x) + motion.x) /
            TILE_SIZE);
        const playerTargetX = Math.floor((playerP.x + (PLAYER_SIZE / 2) * Math.sign(motion.x) + motion.x) /
            TILE_SIZE);
        const playerNextY = Math.floor((playerP.y + (PLAYER_SIZE / 2) * Math.sign(motion.y) + motion.y) /
            TILE_SIZE);
        const playerTargetY = Math.floor((playerP.y + (PLAYER_SIZE / 2) * Math.sign(motion.y) + motion.y) /
            TILE_SIZE);
        ...
        if (currentX < 0) {
            playerP._x = Math.max(PLAYER_SIZE / 2, Math.min(GRID_WIDTH * TILE_SIZE - PLAYER_SIZE / 2, playerP._x + motion._x));
            motion._x = 0;
        }
        ...
        if (motion.y < 0) {
            let upperTile = this.grid.getTile(playerX, Math.floor((playerP.y - PLAYER_SIZE / 2) / TILE_SIZE - 0.1));
    ...
    addDigParticles(player, nextXTile) {
        particles.push(new ParticlesSys({
    ...
    drawBackground() {
        ...
        const celestialY = GRID_HEIGHT * TILE_SIZE * 0.2 * Math.sin(Math.PI * dayProgress);
    ...
    drawUI() {
        ...
        let invTx = "Inventory: " + inventory.getCurrentSize + "/" + inventory.getCapacity;
        ...
        if (this.state === GameState.START_GAME ||
            this.state === GameState.START_DAY ||
            this.state === GameState.END_DAY ||
            this.state === GameState.END_GAME) {
