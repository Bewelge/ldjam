...
export class UpgradeShop extends AbstractShop {
	private selectedUpgrade: string | null
	...
	getSelectedUpgrade(): string | null {
	...
	protected updateContent(): void {
		const contentContainer = this.shopContainer!.querySelector(
			".content-container",
		) as HTMLDivElement
		...
		availableUpgrades.forEach(upgrade => {
			...
			const upgradeItem = this.createElement(
				"div",
				isSelected ? "upgrade-item upgrade-item-selected" : "upgrade-item",
			)
			...
			const levelElement = this.createElement(
				"div",
				"upgrade-level",
				`Level ${upgrade.getLevel}/${upgrade.getMaxLevel}`,
			)
			...
			const effectElement = this.createElement(
				"div",
				"upgrade-effect",
			...
			const costElement = this.createElement(
				"div",
				canAfford ? "upgrade-cost" : "upgrade-cost upgrade-cost-unaffordable",
				upgrade.isMaxLevel ? "MAX LEVEL" : `Cost: ${upgrade.getCurrentCost}`,
			)

			// Purchase button (only if we can afford it)
			if (canAfford && !upgrade.isMaxLevel) {
				const purchaseButton = this.createElement(
					"button",
					"purchase-button",
					"Purchase",
				)
