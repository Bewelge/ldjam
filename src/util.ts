...
export class Vec2 {
	...
	subtractVector(vector: Vec2) {
		this._x -= vector.x
		this._y -= vector.y
		return this
	}
	addAngle(angle: number, dist: number) {
	...
	rotateAround(vec: Vec2, ang: number) {
	...
	translate(ct: CanvasRenderingContext2D) {
		ct.translate(this.x, this.y)
	}
	mirrorAcross(p0: Vec2, p1: Vec2) {
		let vx = p1.x - p0.x
		let vy = p1.y - p0.y
		let x = p0.x - this.x
		let y = p0.y - this.y
	...
	debug(ct: CanvasRenderingContext2D, col = "red") {
		ct.save()
		ct.fillStyle = col
		ct.strokeStyle = col
	...
	static middleOf(vec1: Vec2, vec2: Vec2, a = 0.5) {
		return new Vec2(
			vec1.x * (1 - a) + a * vec2.x,
			vec1.y * (1 - a) + a * vec2.y,
		)
	...
	ellipse(
		ct: Path2D | CanvasRenderingContext2D,
		radX: number,
		radY: number,
		rot = 0,
	) {
	...
	fill(ct: CanvasRenderingContext2D) {
		ct.fill()
		return this
	}
	stroke(ct: CanvasRenderingContext2D) {
		ct.stroke()
		return this
	}
	strokefill(ct: CanvasRenderingContext2D) {
		ct.stroke()
		ct.fill()
		return this
	}
	fillstroke(ct: CanvasRenderingContext2D) {
		ct.fill()
		ct.stroke()
		return this
	}
	path(ct: CanvasRenderingContext2D) {
		ct.beginPath()
		return this
	}
...
function turnTowards(ang0: number, ang1: number) {
	let diff = ang0 - ang1
	if (diff < -PI) {
		diff += PI2
	}
...
function anglePoints(point1: Vec2, point2: Vec2) {
	return Math.atan2(point2.y - point1.y, point2.x - point1.x)
}
function distancePoints(point1: Vec2, point2: Vec2) {
	return Math.sqrt(
...
export function rndArr(list: any[]) {
	return list[rndInt(0, list.length - 1)]
}

export function peg(val: number, min = 0, max = 1) {
	return Math.min(max, Math.max(min, val))
}
